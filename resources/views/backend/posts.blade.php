            ...
            <tbody>
                ...
                <tr wire:key='post_{{ $post->id }}' class="alert" role="alert">
                    ...
                    <td class="status" wire:key='status_{{ $post->id }}'>
                        <span class="{{  $post->status == 'draft' ? 'waiting' : 'active' }}">{{  $post->status }}</span>
                    </td>
                    <td class="status" wire:key='status_{{ $post->id }}'>
                    ...
                    <td wire:key='created_at_{{ $post->id }}'>
                        <span class="fw-bold" aria-hidden="true">{{ $post->created_at->format('m/d/y - h:m') }}</span>
                    </td>
...
function delete_post(id,title) {
    ...
    DeletePost.fire({
        html:'<h5 class="mt-2 mb-1 text-red-500">Are you Sure you want to Delete this Post ?</h5><br><h3>'+title+'</h3><br><span  class="text-muted">Deleted Posts will be moved to Archive, and they will NOT be Visible on the Blog.</span> ',
        showCancelButton: true,
